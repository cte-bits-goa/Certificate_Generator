...
%---------------------------------------------------------------
 ...
 \vfill
 \centering
 \includegraphics[width=\paperwidth,height=\paperheight]{cert.pdf}              % here include background image (You can change the background by changing 'cert.pdf' to desired background
 \vfill
 }}}
%----------------------------------------------------------------
%=============================
...
%=============================
\DTLforeach{name}{
	\name=Name, \course=Dep, \certype=Type}{ %defining variables from the csv file 
	%=============================
	\noindent
	\begin{minipage}[l]{5.7 in}
		\includegraphics[width=.2\linewidth]{cte-logo.png}  %Here include left side image %change width the resize logo
		\\\\\\
		{{
			\onehalfspacing
			\\
			{\bfseries \Huge \textbf{CERTIFICATE OF} \MakeUppercase{\certype}}\\\\%takes Certificate type from csv file and converts it into Upper case
			{\Huge Awarded to}\\\\
			{\Huge\bfseries\color{bluetext} \name}\\
			\\
			\doublespacing
			\arial {\medium For completing the course
				{\course}  which was conducted as a part of the Centre for Technical Education, BITS Pilani Goa Campus from January 2020 to June 2020.
			}
		}}
	\end{minipage}
	\hfill
	...
	{\singlespacing 
		...
		{\hspace{-0.55cm}\shortstack{\vrule width 3.15in height 0.4pt\\\footnotesize}}
		
			{\hspace{-0.55cm}\textbf{\footnotesize PROF. BHARAT M DESHAPANDE (HEAD, CTE)}\\ \vspace{0.3cm}{\hspace{-0.55cm}\color{lightgrey}{BITS PILANI KK BIRLA GOA CAMPUS}}}
